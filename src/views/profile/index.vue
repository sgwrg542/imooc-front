...
watch(isDialogVisible, (val) => {
  if (!val) {
    // 清空文件选择框，防止二次选同一文件不触发change
    inputFileTarget.value.value = null
    // 释放blob，避免内存泄漏和预览残留
    if (currentBlob.value) {
      URL.revokeObjectURL(currentBlob.value)
      currentBlob.value = ''
    }
  }
})
...
/**
 * 数据本地的双向同步（本地草稿，编辑时不直接改 store）
 * 注意：必须浅拷贝，否则引用同一个 store state 对象，改草稿仍会改动 store 并触发持久化
 */
const userInfo = ref({ ...store.getters.userInfo })
/**
 * 头像上传成功回调：立即保存到后端并同步 store（触发持久化），刷新后仍保留
 */
const onUpdateAvatar = async (avatar) => {
  // 1. 先更新本地草稿，让页面立即显示新头像
  userInfo.value.avatar = avatar
  try {
    // 2. 提交后端保存头像
    await putProfile(userInfo.value)
    message('success', '头像上传成功')
    // 3. 同步到 store，触发持久化到 localStorage
...
const onChangeProfile = async () => {
  ...
  try {
    ...
    // 保存成功后再同步到 store（触发持久化到 localStorage）
    store.commit('user/setUserInfo', userInfo.value)
  } finally {
    loading.value = false
  }
